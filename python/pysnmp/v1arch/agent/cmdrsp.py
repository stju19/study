...
class SysDescr:
    ...
    def __call__(self, protoVer, msg=None):
        if msg:
            self.msg = msg
        else:
            self.msg = self.msg if self.msg else api.protoModules[
                protoVer].OctetString('PySNMP example command responder')
        return self.msg
...
def cbFun(transportDispatcher, transportDomain, transportAddress, wholeMsg):
    while wholeMsg:
        ...
        if reqPDU.isSameTypeWith(pMod.GetNextRequestPDU()):
            ...
            for oid, val in pMod.apiPDU.getVarBinds(reqPDU):
                if oid in mibInstrIdx:
                    varBinds.append((oid, mibInstrIdx[oid](msgVer, val)))
                else:
                    # No such instance
                    varBinds.append((oid, val))
                    pendingErrors.append(
