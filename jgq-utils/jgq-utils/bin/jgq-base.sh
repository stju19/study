...
yum_repo="http://10.43.211.43/podm_install/"
...
EOF
    ...
    run $container_name "yum -y install bash-completion tar passwd createrepo net-tools httpd perl vim perl-DBI nmap-ncat libaio \
        ...
        sudo openssh-server openssh-clients python-dateutil python-memcached vsftpd net-snmp net-snmp-utils web.py man-db zip unzip \
        ntp chrony telnet"
    ...
    run $container_name "chmod 600 /root/.ssh/authorized_keys"
    ...
    run $container_name "route add default gw 138.1.1.20"
    make_bin
    docker cp $podm_path/target/podmanager_*_el7_x86_64.bin $container_name:/home/podm.bin
    make_clean
    run $container_name "/home/podm.bin -- --docker install"
